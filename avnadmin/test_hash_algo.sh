...
# --- (A) default (sha1) repo ---
REPO_SHA1=/tmp/avn_test_hash_repo_sha1
rm -rf "$REPO_SHA1"
...
# --- (B, C) sha256 repo ---
REPO_SHA256=/tmp/avn_test_hash_repo_sha256
rm -rf "$REPO_SHA256"
...
"$SERVER_BIN" demo "$REPO_SHA256" "$PORT" >/tmp/avn_test_hash_srv.log 2>&1 &
SRV=$!
sleep 1.2
...
if "$ADMIN_BIN" create /tmp/avn_test_hash_bad --algos md5 2>/dev/null; then
    echo "  FAIL unknown algo accepted"
    FAILS=$((FAILS+1))
else
    echo "  ok   unknown algo rejected"
fi
rm -rf /tmp/avn_test_hash_bad
