...
int line_is_valid(const char*, int, int);
int locate_line_start(const char*, int);
int locate_line_end(const char*, int);
...
// Exported:
int dir_count_entries(const char* body) {
...
int line_is_valid(const char* body, int start, int end) {
if ((end - start) < 5) {
        {
            return 0;
        }
    }
if (string_char_at(body, (start + 1)) != 32) {
        {
            return 0;
        }
    }
...
int locate_line_start(const char* body, int target) {
int n = string_length(body);
if (n == 0) {
        {
            return -1;
...
int locate_line_end(const char* body, int target) {
int n = string_length(body);
if (n == 0) {
        {
            return -1;
...
// Exported:
int dir_entry_kind(const char* body, int i) {
int start = locate_line_start(body, i);
if (start < 0) {
        {
            return 0;
        }
    }
    return string_char_at(body, start);
}

// Exported:
const char* dir_entry_sha(const char* body, int i) {
int start = locate_line_start(body, i);
if (start < 0) {
        {
            return "";
...
int end = locate_line_end(body, i);
int j = (start + 2);
while (j < end) {
        {
if (string_char_at(body, j) == 32) {
                {
                    break;
                }
            }
j = (j + 1);
    ...
    return string_substring(body, (start + 2), j);
}

// Exported:
const char* dir_entry_name(const char* body, int i) {
int start = locate_line_start(body, i);
if (start < 0) {
        {
            return "";
...
if (j >= end) {
    ...
    return string_substring(body, (j + 1), end);
}
...
int32_t aether_dir_count_entries(const char* body) {
    return dir_count_entries(body);
}
int32_t aether_line_is_valid(const char* body, int32_t start, int32_t end) {
    return line_is_valid(body, start, end);
}
int32_t aether_locate_line_start(const char* body, int32_t target) {
    return locate_line_start(body, target);
}
int32_t aether_locate_line_end(const char* body, int32_t target) {
    return locate_line_end(body, target);
}
int32_t aether_dir_entry_kind(const char* body, int32_t i) {
    return dir_entry_kind(body, i);
}
const char* aether_dir_entry_sha(const char* body, int32_t i) {
    return dir_entry_sha(body, i);
}
const char* aether_dir_entry_name(const char* body, int32_t i) {
    return dir_entry_name(body, i);
}
