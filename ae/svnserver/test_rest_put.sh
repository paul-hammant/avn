...
tlib_check "GET body"    "int main() { return 42; }" "$body"
...
code=$(curl -s -o /tmp/put_resp -w '%{http_code}' -X PUT \
         ...
         "$URL/path/src/main.c")
tlib_check "PUT ok status"  "201" "$code"
tlib_check "PUT body has rev" "1" "$(grep -c '"rev":' /tmp/put_resp || true)"
# Content round-trip.
body=$(curl -s "$URL/path/src/main.c")
tlib_check "PUT content visible" "updated content" "$body"
...
code=$(curl -s -o /tmp/put_resp -D /tmp/put_hdrs -w '%{http_code}' -X PUT \
         ...
         "$URL/path/src/main.c")
tlib_check "stale PUT 409"  "409" "$code"
tlib_check "stale PUT hdr"  "1"   "$(grep -ci 'X-Svnae-Current-Hash' /tmp/put_hdrs || true)"
...
tlib_check "PUT new 201"    "201" "$code"
body=$(curl -s "$URL/path/newfile.txt")
tlib_check "new file content" "brand new" "$body"

# --- (E) PUT existing file without based-on → 409. ---
code=$(curl -s -o /tmp/put_resp -w '%{http_code}' -X PUT \
         --data-binary @/tmp/put_body \
         "$URL/path/newfile.txt")
tlib_check "PUT exists without hdr 409" "409" "$code"
...
code=$(curl -s -o /dev/null -w '%{http_code}' -X PUT \
         ...
         -H "Svn-Based-On: $sha" \
         "$URL/path/src/main.c")
tlib_check "bob PUT denied 403"  "403" "$code"
...
code=$(curl -s -o /tmp/put_resp -w '%{http_code}' -X PUT \
         ...
         -H "Svn-Based-On: $sha" \
         "$URL/path/src/main.c")
tlib_check "alice PUT ok"   "201" "$code"
...
tlib_check "author = alice" "1" "$(echo "$log" | grep -c '"author":"alice"' || true)"
...
code=$(curl -s -o /tmp/put_resp -w '%{http_code}' -T /tmp/upload_me \
         ...
         -H "Svn-Based-On: $sha" \
         "$URL/path/src/main.c")
tlib_check "curl -T 201"   "201" "$code"
