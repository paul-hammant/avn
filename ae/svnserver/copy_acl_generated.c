...
int starts_with_same_bytes(const char*, const char*, int);
...
// Exported:
const char* copy_acl_follow(const char* body, const char* from_path, const char* to_path) {
...
int from_len = string_length(from_path);
const char* out = "";
...
if ((sp > line_start) && (sp < i)) {
                                {
sha = string_substring(body, line_start, sp);
this_path = string_substring(body, (sp + 1), i);
out = string_concat(out, sha);
out = string_concat(out, " ");
out = string_concat(out, this_path);
out = string_concat(out, "\n");
rebase = 0;
pl = string_length(this_path);
if (pl == from_len) {
                                        {
if (string_equals(this_path, from_path) == 1) {
                                                {
rebase = 1;
                                                }
                                            }
...
if (pl > from_len) {
                                                {
if (string_char_at(this_path, from_len) == 47) {
                                                        {
...
if (pl == from_len) {
                                                {
out = string_concat(out, to_path);
                                                }
                                            } else {
                                                {
tail = string_substring(this_path, (from_len + 1), pl);
out = string_concat(out, to_path);
out = string_concat(out, "/");
...
int starts_with_same_bytes(const char* hay, const char* needle, int n) {
int i = 0;
while (i < n) {
        {
if (string_char_at(hay, i) != string_char_at(needle, i)) {
                {
                    return 0;
                }
            }
...
const char* aether_copy_acl_follow(const char* body, const char* from_path, const char* to_path) {
    return copy_acl_follow(body, from_path, to_path);
}
int32_t aether_starts_with_same_bytes(const char* hay, const char* needle, int32_t n) {
    return starts_with_same_bytes(hay, needle, n);
}
