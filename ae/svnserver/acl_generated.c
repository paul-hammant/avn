...
// Exported:
int acl_decide(const char* body, const char* user, int want_write) {
...
if (i > line_start) {
                        {
sign = string_char_at(body, line_start);
if ((sign == 43) || (sign == 45)) {
                                {
...
name = string_substring(body, name_start, name_end);
has_r = 1;
has_w = 1;
if (colon_at >= 0) {
                                        {
...
if (is_wild == 0) {
                                        {
if (string_length(name) == ulen) {
                                                {
if (string_equals(name, user) == 1) {
                                                        {
is_user = 1;
                                                        }
                                                    }
...
int32_t aether_acl_decide(const char* body, const char* user, int32_t want_write) {
    return acl_decide(body, user, want_write);
}
