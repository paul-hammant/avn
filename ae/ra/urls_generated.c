...
const char* rev_prefix(const char*, const char*, int);
const char* u_int_to_dec(int);
const char* u_digit_char(int);
...
// Exported:
const char* url_rev_info(const char* base, const char* repo, int rev) {
...
// Exported:
const char* url_rev_paths(const char* base, const char* repo, int rev) {
const char* out = rev_prefix(base, repo, rev);
    int _heap_out = 0; (void)_heap_out;
    return string_concat(out, "/paths");
}

// Exported:
const char* url_rev_cat(const char* base, const char* repo, int rev, const char* path) {
const char* out = rev_prefix(base, repo, rev);
    int _heap_out = 0; (void)_heap_out;
out = string_concat(out, "/cat/");
    return string_concat(out, path);
}

// Exported:
const char* url_rev_list(const char* base, const char* repo, int rev, const char* path) {
const char* out = rev_prefix(base, repo, rev);
    int _heap_out = 0; (void)_heap_out;
if (string_length(path) == 0) {
        {
            return string_concat(out, "/list");
        }
    }
out = string_concat(out, "/list/");
    return string_concat(out, path);
}

// Exported:
const char* url_rev_props(const char* base, const char* repo, int rev, const char* path) {
const char* out = rev_prefix(base, repo, rev);
    int _heap_out = 0; (void)_heap_out;
if (string_length(path) == 0) {
        {
            return string_concat(out, "/props");
        }
    }
...
// Exported:
const char* url_rev_blame(const char* base, const char* repo, int rev, const char* path) {
const char* out = rev_prefix(base, repo, rev);
    int _heap_out = 0; (void)_heap_out;
out = string_concat(out, "/blame/");
    return string_concat(out, path);
}
...
const char* out = string_concat(base, "/repos/");
    int _heap_out = 0; (void)_heap_out;
out = string_concat(out, repo);
out = string_concat(out, "/branches/");
out = string_concat(out, branch_name);
    return string_concat(out, "/create");
}

const char* rev_prefix(const char* base, const char* repo, int rev) {
const char* out = string_concat(base, "/repos/");
    int _heap_out = 0; (void)_heap_out;
out = string_concat(out, repo);
out = string_concat(out, "/rev/");
    return string_concat(out, u_int_to_dec(rev));
}

const char* u_int_to_dec(int v) {
if (v == 0) {
        {
            return "0";
        }
...
while (n > 0) {
        {
d = (n - ((n / 10) * 10));
out = string_concat(u_digit_char(d), out);
n = (n / 10);
        }
    }
if (neg == 1) {
...
const char* aether_url_rev_info(const char* base, const char* repo, int32_t rev) {
    return url_rev_info(base, repo, rev);
}
const char* aether_url_rev_paths(const char* base, const char* repo, int32_t rev) {
    return url_rev_paths(base, repo, rev);
}
const char* aether_url_rev_cat(const char* base, const char* repo, int32_t rev, const char* path) {
    return url_rev_cat(base, repo, rev, path);
}
const char* aether_url_rev_list(const char* base, const char* repo, int32_t rev, const char* path) {
    return url_rev_list(base, repo, rev, path);
}
const char* aether_url_rev_props(const char* base, const char* repo, int32_t rev, const char* path) {
    return url_rev_props(base, repo, rev, path);
}
...
const char* aether_rev_prefix(const char* base, const char* repo, int32_t rev) {
    return rev_prefix(base, repo, rev);
}
const char* aether_u_int_to_dec(int32_t v) {
    return u_int_to_dec(v);
}
const char* aether_u_digit_char(int32_t d) {
    return u_digit_char(d);
}
