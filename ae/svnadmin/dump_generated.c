...
// Exported:
const char* dump_prelude(int head, int rev_count, int rep_count) {
const char* out = "SVNAE-DUMP 1\nFORMAT svnae-fsfs-1\nHEAD ";
    int _heap_out = 0; (void)_heap_out;
out = string_concat(out, int_to_dec(head));
out = string_concat(out, "\nREV-COUNT ");
out = string_concat(out, int_to_dec(rev_count));
out = string_concat(out, "\nREP-COUNT ");
out = string_concat(out, int_to_dec(rep_count));
out = string_concat(out, "\n");
    return out;
}

// Exported:
const char* rep_header(const char* sha, int size) {
const char* out = "REP ";
    int _heap_out = 0; (void)_heap_out;
out = string_concat(out, sha);
out = string_concat(out, "\nSIZE ");
out = string_concat(out, int_to_dec(size));
out = string_concat(out, "\n");
    return out;
}

// Exported:
const char* rev_pointer_block(int rev, const char* pointer_sha) {
const char* out = "REV ";
    int _heap_out = 0; (void)_heap_out;
out = string_concat(out, int_to_dec(rev));
out = string_concat(out, "\nPOINTER ");
out = string_concat(out, pointer_sha);
out = string_concat(out, "\n");
    return out;
}
...
const char* aether_dump_prelude(int32_t head, int32_t rev_count, int32_t rep_count) {
    return dump_prelude(head, rev_count, rep_count);
}
const char* aether_rep_header(const char* sha, int32_t size) {
    return rep_header(sha, size);
}
const char* aether_rev_pointer_block(int32_t rev, const char* pointer_sha) {
    return rev_pointer_block(rev, pointer_sha);
}
